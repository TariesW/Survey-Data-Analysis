...
lm_positive_confidence_ex = lm( confidence ~ `Extension_Scale_Total_Score`, data = study1_positive)
summary(lm_positive_confidence)
coef(lm_positive_confidence)
confint(lm_positive_confidence)
ggplot(data = study1_positive, aes(y=confidence, x = `Extension_Scale_Total_Score`))+ 
  geom_point()+
  geom_smooth(method = lm)

# linear regression: negative condition confidence with extension scale
lm_negative_confidence_ex = lm( confidence ~ `Extension_Scale_Total_Score`, data = study1_negative)
summary(lm_negative_confidence_ex)
coef(lm_negative_confidence_ex)
confint(lm_negative_confidence_ex)
ggplot(data = study1_negative, aes(y=confidence, x = `Extension_Scale_Total_Score`)) + 
  geom_point()+
  geom_smooth(method = lm)
...
lm_frequency_confidence_p = lm( `Study2-F-confident` ~ `Extension_Scale_Total_Score`, data = study2_percentage)
summary(lm_frequency_confidence_p)
coef(lm_frequency_confidence_p)
confint(lm_frequency_confidence_p)
ggplot(data = study2_percentage, aes(y=`Study2-F-confident`, x= `Extension_Scale_Total_Score`)) + 
  geom_point()+
  geom_smooth(method=lm)

# linear regression: confidence with extension scale under frequency condition
lm_frequency_confidence_f = lm( `Study2-F-confident` ~ `Extension_Scale_Total_Score`, data = study2_frequency)
summary(lm_frequency_confidence_f)
coef(lm_frequency_confidence_f)
confint(lm_frequency_confidence_f)
ggplot(data = study2_frequency, aes(y=`Study2-F-confident`, x= `Extension_Scale_Total_Score`)) + 
  geom_point()+
  geom_smooth(method=lm)
...
chisq$observed #observe counts
chisq$expected #expected counts
chisq$p.value  #print p values
...
cor.test(study3_bowl_type$`Study 3_Bowl type`, study3_bowl_type$Replicate_Scale_Total_Score, 
                method = "pearson")


# Correlation: Bowl choice and extension scale
cor.test(study3_bowl_type$`Study 3_Bowl type`, study3_bowl_type$Extension_Scale_Total_Score, 
                method = "pearson")


# Linear regression: affect precision(clear) for Bowl A and replication scale
...
# Linear regression: affect precision(clear) for Bowl B and replication scale
...
# Linear regression: affect(feeling) for Bowl A and replication scale
...
# Linear regression: affect(feeling) for Bowl B and replication scale
...
# linear regression: affect precision(clear) for Bowl A and extension scale
lm_cleara_extension = lm( Extension_Scale_Total_Score ~ `Study 3_A_Clear`, data = study3_bowl_clearness)
summary(lm_cleara_extension)
coef(lm_cleara_extension)
confint(lm_cleara_extension)
ggplot(data = study3_bowl_clearness, aes(y=Extension_Scale_Total_Score, x= `Study 3_A_Clear`)) + 
  geom_point()+
  geom_smooth(method=lm)


# linear regression: affect precision(clear) for Bowl B and extension scale
...
# Compare linear models: affect precision(clear) for Bowl A&B with extension scale


# linear regression: affect(feeling) for Bowl A and extension scale
lm_feelinga_extension = lm( Extension_Scale_Total_Score ~ `Study 3_A_Feeling`, data = study3_bowl_feeling)
summary(lm_feelinga_extension)
coef(lm_feelinga_extension)
confint(lm_feelinga_extension)
ggplot(data = study3_bowl_feeling, aes(y=Extension_Scale_Total_Score, x= `Study 3_A_Feeling`)) + 
  geom_point()+
  geom_smooth(method=lm)

# linear regression: affect(feeling) for Bowl B and extension scale
lm_feelingb_extension = lm( Extension_Scale_Total_Score ~ `Study 3_B_Feeling`, data = study3_bowl_feeling)
summary(lm_feelingb_extension)
coef(lm_feelingb_extension)
confint(lm_feelingb_extension)
ggplot(data = study3_bowl_feeling, aes(y=Extension_Scale_Total_Score, x= `Study 3_B_Clear`)) + 
  geom_point()+
  geom_smooth(method=lm)
...
lm_N_confidence_ex = lm( `Study4-NL-confident` ~ `Extension_Scale_Total_Score`, data = study4_N)
summary(lm_N_confidence_ex)
coef(lm_N_confidence_ex)
confint(lm_N_confidence_ex)
ggplot(data = study4_N, aes(y=`Study4-NL-confident`, x= `Extension_Scale_Total_Score`)) + 
  geom_point()+
  geom_smooth(method=lm)

# linear regression: confidence Loss condition with extension scale
lm_N_confidence_L_ex  = lm( `Study4-NL-confident` ~ `Extension_Scale_Total_Score`, data = study4_L)
summary(lm_N_confidence_L_ex)
coef(lm_N_confidence_L_ex)
confint(lm_N_confidence_L_ex)
ggplot(data = study4_N, aes(y=`Study4-NL-confident`, x= `Extension_Scale_Total_Score`)) + 
  geom_point()+
  geom_smooth(method=lm)
